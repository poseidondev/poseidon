...
#if defined(NDEBUG)
# error "Poseidon cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xeb0f25e0ef83aab512ca85cb5aa3bd57c7e1fb68a8dd0db198c71cb08a8d538b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Poseidon: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Poseidon Signed Message:\n";
...
{
    // Poseidon: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Poseidon: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Poseidon: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1514491228, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "CNN 28 Dec 2017  Dozens killed as blasts rock Kabul";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0432a8c0fa6fa793d7f71d7d1b0ee480e2548088014a33d40111bd20c1f99df0ffb15edba3089a8f2f0465ee8bc7b3a41373e7fe01bb0ffb1abe1f8bc8dec2cc87") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1514491228;
        ...
        assert(block.hashMerkleRoot == uint256("0xa31c240e358fc08098c07716b093cf45a78308ddd24bee6b7c0abdf07319d698"));
...
unsigned char pchMessageStart[4] = { 0xdc, 0x03, 0xd5, 0xde }; // Poseidon: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PoseidonMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PoseidonMiner(CWallet *pwallet)
{
    printf("PoseidonMiner started\n");
    ...
    try { loop {
        ...
        printf("Running PoseidonMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PoseidonMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PoseidonMiner, pwallet));
}
